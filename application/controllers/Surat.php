...
class Surat extends CI_Controller {
	...
	// BATAS AWAL UBAH STATUS UNTUK KERJA PRAKTEK
...
".$this->session->userdata('nama_lengkap')." - TU ".$this->session->userdata('fakultas')
	...
	// BATAS AKHIR UBAH STATUS UNTUK KERJA PRAKTEK


	// BATAS AWAL UBAH STATUS UNTUK TUGAS AKHIR
	...
	public function ubahFinishta($id_surat)
	{
		$this->statussurat_model->SuratTAToFinish($id_surat);
		$kirimemail = $this->tampilsurat_model->GetIdentitasMahasiswa($id_surat);
		...
		$isi       = html_entity_decode("
			...
			Berdasarkan pengajuan surat Anda, kami TU FASILKOM ingin menginformasikan bahwa Surat Penelitian Tugas Akhir yang Anda ajukan telah selesai. Anda dapat mengambil Surat Penelitian Tugas Akhir yang Anda ajukan dengan datang ke TU FASILKOM di jam operasional kami yaitu:
			Senin – Sabtu dari jam 08:00 WIB hingga 16:00 WIB
			Adapun yang dapat mengambil Surat Penelitian Tugas Akhir yaitu Nama Mahasiswa yang mengajukan permohonan Surat Penelitian Tugas Akhir dengan membawa Tanda Terima yang Anda bisa unduh di dalam lampiran surel ini.
			Anda dapat mengecek status permohonan pembuatan Surat Penelitian Tugas Akhir Anda melalui login di situs E-Surat: <a href='http://www.esurat.mercubuana.ac.id'>www.esurat.mercubuana.ac.id</a>
			Terima kasih.
			<br><br>
			Salam,
			<br><br>
	    ...
	    $data = array(
	    	...
	    	'tanggal_diajukan' =>$kirimemail->tanggal_diajukan,
	    	'no_surat'   => $kirimemail->no_surat,
	    	'nama_perusahaan' => $kirimemail->nama_perusahaan,
	    	'alamat_perusahaan'=>$kirimemail->alamat_perusahaan
	    );
	    //Load html view
	    $this->html2pdf->html($this->load->view('tiket/pdfta', $data, true));
	    ...
	    if($path = $this->html2pdf->create('save')) {
			...
			$this->session->set_flashdata('info','true');
			redirect('admin/prosesta');
		}
		
	}

	public function ubahAmbilTa($id_surat)
	{
		$this->statussurat_model->SuratTAToTake($id_surat);

		$this->session->set_flashdata('info','true');
	    redirect('admin/finishta');
	}

	public function kirimpesantolakta()
	{
			$idsurat = $this->input->post('id_surat');
		$data = $this->tampilsurat_model->detailta($idsurat);
		...
		$isi= html_entity_decode(
          ...
          Berdasarkan pengajuan surat Anda, kami TU ".$this->session->userdata('fakultas')." tidak dapat memproses Surat Tugas Akhir yang Anda ajukan karena terdapat kesalahan dalam proses pengisian data yaitu <B><font color='red'>". $pesan ."</font></B> pada saat Anda mengisi formulir pengajuan surat. Pastikan Anda mengisi data yang sesuai dengan kaidah penulisan alamat yang benar serta menuliskan nama jabatan yang sesuai dengan jabatan orang yang Anda tuju.
          Anda dapat mengajukan permohonan pembuatan Surat Tugas Akhir kembali melalui situs E-Surat: www.esurat.mercubuana.ac.id
          <br><br>
          Terima kasih.
          <br><br>
          Salam,
		...
		$this->statussurat_model->SuratTAToTolak($data['id_surat']);

		$this->session->set_flashdata('infotolak','true');
	    redirect('admin/waitingta');
	}

	// BATAS AKHIR UBAH STATUS UNTUK TUGAS AKHIR
